...
export default function DraggableResizableIcon({ item, onUpdate, onRemove }: DraggableResizableIconProps) {
  ...
  const [interactionState, setInteractionState] = useState({ isDragging: false, isResizing: false, isRotating: false });
  const [isActive, setIsActive] = useState(false);
  const nodeRef = useRef<HTMLDivElement>(null);
  
  const interactionDetails = useRef({
    startX: 0, startY: 0, startLeft: 0, startTop: 0, startSize: 0, startRotation: 0,
    centerX: 0, centerY: 0,
  });

  const handleInteractionStart = (e: MouseEvent<HTMLDivElement>, type: 'drag' | 'resize' | 'rotate') => {
    e.preventDefault();
    e.stopPropagation();
    ...
    interactionDetails.current = {
      ...
      startSize: item.size, startRotation: item.rotation,
  ...
  useEffect(() => {
    const handleMouseMove = (e: globalThis.MouseEvent) => {
      ...
      const { startX, startY, startLeft, startTop, startSize, centerX, centerY } = interactionDetails.current;
    ...
    const handleMouseUp = () => {
      setInteractionState({ isDragging: false, isResizing: false, isRotating: false });
    };
  ...
  const isInteracting = interactionState.isDragging || interactionState.isResizing || interactionState.isRotating;
