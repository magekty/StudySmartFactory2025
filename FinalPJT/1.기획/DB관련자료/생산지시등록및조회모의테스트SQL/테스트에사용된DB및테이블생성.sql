-- MySQL Database DDL Script for MES_DB (최종 수정 및 최적화 버전)
...
-- =============================================================
-- 1단계: 외래키(FOREIGN KEY)를 참조받지 않는 독립적인 테이블들 생성
-- (공통 코드, 사용자, 역할, 조직, 메뉴, 품목, 작업장, 창고, 생산 라인, 문서)
-- =============================================================
...
-- 11. TB_DOCUMENT_MANAGEMENT (문서 관리)
...
-- =============================================================
-- 2단계: 1단계 테이블을 참조하는 테이블들 생성
-- (BOM, 원자재-제품 매핑, 라우팅, 생산 라인, 설비, 생산 계획, 검사 계획, 품질 점검표)
-- =============================================================

-- 12. TB_MES_BOM (BOM 마스터)
...
-- 13. TB_COMPONENT_PRODUCT_MAPPING (원자재-제품 매핑)
...
-- 19. TB_QUALITY_CHECKLIST (품질 점검표)
...
-- =============================================================
-- 3단계: 2단계 테이블을 참조하는 테이블들 생성
-- (라우팅 공정, 설비 파라미터, 작업 지시, 생산 지시 등)
-- =============================================================

-- 20. TB_ROUTE_OPERATION (라우팅 공정)
...
-- 21. TB_PARAMETER_MANAGEMENT (설비 파라미터)
...
-- 23. TB_QUALITY_CHECKLIST_ITEM (품질 점검 항목)
...
-- =============================================================
-- 4단계: 3단계 테이블을 참조하는 테이블들 생성
-- (생산 지시, 재고 이동, 검사 결과 등)
-- =============================================================

-- 26. TB_PRODUCTION_ORDER (생산 지시)
...
-- 30. TB_REPORT_MANAGEMENT (리포트 관리)
...
-- =============================================================
-- 5단계: 4단계 테이블을 참조하는 테이블들 생성
-- (생산 실적, 검사 불량, 품질 점검 결과, 설비 보전 등)
-- =============================================================

-- 31. TB_PRODUCTION_PERFORMANCE (생산 실적)
...
-- 32. TB_PRODUCTION_HISTORY (생산 이력)
...
-- 35. TB_QUALITY_CHECK_RESULT (품질 점검 결과)
...
-- 36. TB_EQUIPMENT_MAINTENANCE (설비 보전)
...
-- 37. TB_EQUIPMENT_DOWNTIME (설비 비가동)
...
-- 44. TB_ANALYSIS_DOWNTIME (비가동 분석)
...
-- 45. TB_USER_LOGIN_LOG (사용자 로그인 로그)
...
-- 46. TB_SYSTEM_AUDIT_LOG (시스템 감사 로그)
...
-- 47. TB_EQUIPMENT_SENSOR_DATA (설비 센서 데이터)
