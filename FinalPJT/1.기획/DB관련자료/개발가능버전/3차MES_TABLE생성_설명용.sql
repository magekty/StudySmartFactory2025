...
CREATE TABLE `TB_SHIFT_ASSIGNMENT` (
  ...
  `shift_date` DATE NOT NULL COMMENT '근무 날짜',
  `shift_id` BIGINT NOT NULL COMMENT '교대 ID (FK)',
  `worker_id` VARCHAR(36) NOT NULL COMMENT '작업자 ID (FK→TB_USER.user_id 권장)',
  ...
  CONSTRAINT `ck_assign_time_order`
    CHECK (`end_ts` > `start_ts`)
) ENGINE=InnoDB COMMENT='교대 배치: 날짜/교대/작업자 × (설비 또는 작업장) 다건 배정(UTC). XOR+부분 유니크로 중복 차단.';
-- Batch 3: 핵심 마스터 테이블
...
CREATE TABLE `TB_PRODUCTION_PERFORMANCE` (
  ...
  `worker_id` VARCHAR(50) NULL COMMENT '작업자 ID(선택)',
  `is_deleted` TINYINT DEFAULT 0 COMMENT '소프트삭제 플래그',
  `deleted_at` DATETIME NULL COMMENT 'UTC',
  `created_by` VARCHAR(50) NOT NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'UTC',
  ...
  -- 선택: good_qty 생성 컬럼 필요 시 아래 주석 해제
  -- , `good_qty` DECIMAL(10,4) GENERATED ALWAYS AS (GREATEST(`produced_qty` - `defect_qty`, 0)) STORED
) ENGINE=InnoDB COMMENT='생산 실적: 작업 지시 기준 시간/수량 기록. 수량/시간 무결성과 WO·설비 타임라인 최적화(UTC).';
...
) ENGINE=InnoDB COMMENT='검사: 품질 검사 본체. 품목/유형/시간 인덱싱, 대상(지시/LOT/공정/설비) 선택 FK(UTC).';
...
) ENGINE=InnoDB COMMENT='부적합: 불량 1:1 확장. 보고일 조회 최적화, 텍스트 상세(UTC).';
