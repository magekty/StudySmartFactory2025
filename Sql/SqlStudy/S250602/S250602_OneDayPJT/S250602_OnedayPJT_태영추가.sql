...
#######################김태영추가#########################
update users set role = 'admin' where username = 'root';	# role 관리자로 변경
CREATE USER 'staff'@'localhost' IDENTIFIED BY 'sm^xpq$9482@@';	# staff db접근 계정비번
CREATE USER 'admin'@'localhost' IDENTIFIED BY 'Ex@MpL3-pA55w0rD.sTr0nG!7#';	# admin db접근 계정비번
GRANT SELECT ON restaurant.* TO 'staff'@'localhost';	# staff는 select 권한만
GRANT ALL PRIVILEGES ON restaurant.* TO 'admin'@'localhost';	# admin은 모든 권한
FLUSH PRIVILEGES;	# 즉시 권한 반영
GRANT EXECUTE ON PROCEDURE `restaurant`.`pay_order` TO 'staff'@'localhost' ;	# staff 프로시져 사용 권한 
FLUSH PRIVILEGES;	# 즉시 권한 반영
GRANT EXECUTE ON PROCEDURE `restaurant`.`pay_order` TO 'admin'@'localhost' ;	# admin 프로시져 사용 권한 
FLUSH PRIVILEGES;	# 즉시 권한 반영